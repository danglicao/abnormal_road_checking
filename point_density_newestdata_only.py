    ...
    for folder_name in folder_list:
        ...
        try:
            ...
            if files_in_folder:
                ...
                files_in_folder.sort(key=lambda f: os.path.getmtime(os.path.join(folder_path, f)))

                # Select the most recent file
                latest_file_name = files_in_folder[-1]
                latest_file_path = os.path.join(folder_path, latest_file_name)

                try:
                    # Convert WKT to a readable list of points
                    point_list = get_point_data(latest_file_path)

                    # Calculate and output point density
                    output_file_name = f"density_of_{os.path.splitext(latest_file_name)[0]}.csv"
                    written_path = os.path.join(output_folder, output_file_name)
                    get_point_density(point_list, polygon_coords, written_path, area)
                    print(f'Finish processing file: {latest_file_name}')
